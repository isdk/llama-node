...
describe("stableCode", () => {
    describe("model options", () => {
        describe("Resolve the correct number of GPU layers", async () => {
            ...
            const llamaMock = new Proxy(llama, {
                get(target, prop, receiver) {
                    if (prop === "gpuSupportsMmap")
                        return false;
            ...
            const ggufInsights = await GgufInsights.from(fileInfo, llamaMock);
            ...
            it("attempts to resolve 33 gpuLayers", async () => {
                {
                    ...
                    expect(res.contextSize).to.greaterThanOrEqual(11264);
                }
                try {
            ...
            it('attempts to resolve "max"', async () => {
                ...
                }
                {
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(6912);
                }
                {
                    const res = await resolveGpuLayers("max", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(7936);
                }
            });
            ...
            it('attempts to resolve "auto"', async () => {
                ...
                }
                {
                    ...
                    expect(res.contextSize).to.greaterThanOrEqual(10841);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.contextSize).to.greaterThanOrEqual(8521);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.contextSize).to.greaterThanOrEqual(6206);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.contextSize).to.greaterThanOrEqual(1517);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(3328);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(3840);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(4352);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(5376);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(5888);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(6656);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(7168);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                ...
                }
                {
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(9216);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.greaterThanOrEqual(10752);
                }
                {
                    const res = await resolveGpuLayers("auto", {
                        totalVram: s1GB * 6,
                    ...
                    expect(res.gpuLayers).to.eql(33);
                    expect(res.contextSize).to.be.greaterThan(10000);
                }
            });
            ...
            it("attempts to resolve {min?: number, max?: number}", async () => {
                ...
                }
                {
                    ...
                    expect(res.gpuLayers).to.be.gte(16);
                    expect(res.gpuLayers).to.be.lte(24);
                    expect(res.gpuLayers).to.be.within(16, 24);
                    expect(res.contextSize).to.be.greaterThan(7000);
                }
            });
            ...
            it("attempts to resolve {fitContext?: {contextSize?: number}}", async () => {
                ...
                }
                {
                    ...
                    expect(res.gpuLayers).to.be.gte(30);
                    expect(res.contextSize).to.be.greaterThan(5000);
                    expect(res.contextSize).to.be.gte(contextSize);
                }
                {
                    const contextSize = 4096;
                ...
                }
                {
                    ...
                    expect(res.gpuLayers).to.be.within(1, 8);
                    expect(res.contextSize).to.be.greaterThan(8500);
                    expect(res.contextSize).to.be.gte(contextSize);
                }
                {
                    const contextSize = 8192;
