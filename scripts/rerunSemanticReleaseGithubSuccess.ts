...
const context = {
    ...
    options: {
        repositoryUrl: "https://github.com/isdk/llama-node.git"
    },
    logger: console,
    env: process.env
};
...
for (const release of context.releases) {
    if (release.pluginName === "@semantic-release/npm") {
        if (release.url == null) {
            ...
            release.url = "https://www.npmjs.com/package/@isdk/llama-node/v/" + release.version;
        }
    } else if (release.pluginName === "@semantic-release/github") {
        if (release.url == null) {
            release.name = "GitHub release";
            release.url = "https://github.com/isdk/llama-node/releases/tag/" + release.gitTag;
        }
    }
}
